...
if ($dir != false) {
    ...
    // insert 
    $insert = "INSERT INTO accommodations (accomodation_id,title,subtitle,description,price,images_uri,slug,status) 
    VALUES('$accomodation_id','$title','$subtitle','$description','$price','$dir','$slug','$status')";

    if (mysqli_query($con, $insert)) {

        $res->success = true;

        $res->message = "Accomodation Created Successfully";
    } else {
        $res->success = false;
    ...
    $res->message = "There Was An Error Uploading Accomodation Files";
}

$response=new stdClass();

$response->message="Create Accomodation";
...
function upload_gallery_images($files, $name)
{
    ...
    $oldmask = umask(0);
    mkdir('./uploads/accommodations/' . $folder_name . '/');
    umask($oldmask);

    $base_dir = "uploads/accommodations/" . $folder_name . '/';
    ...
    foreach ($files as $file) {
        ...
        if ($uploadOk == 0) {
            ...
            // print_r($file);
            if (move_uploaded_file($file['tmp_name'], 'uploads/accommodations/' . $folder_name . '/' . $image_name)) {
