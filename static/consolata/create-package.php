...
if ($dir != false) {
    ...
    // insert 
    $insert = "INSERT INTO packages (package_id,title,subtitle,description,price,images_uri,slug,status) 
    VALUES('$package_id','$title','$subtitle','$description','$price','$dir','$slug','$status')";
    ...
    $res->message = "There Was An Error Uploading package Files";
}

$response=new stdClass();

$response->message="Create Package";
...
function upload_gallery_images($files, $name)
{
    ...
    $oldmask = umask(0);
    mkdir('./uploads/packages/' . $folder_name . '/');
    umask($oldmask);

    $base_dir = "uploads/packages/" . $folder_name . '/';
    ...
    foreach ($files as $file) {
        ...
        if ($uploadOk == 0) {
            ...
            // print_r($file);
            if (move_uploaded_file($file['tmp_name'], 'uploads/packages/' . $folder_name . '/' . $image_name)) {
