...
$response->detail = HousesInterface($con, $REQ_OBJ);

echo json_encode($response);


function HousesInterface($con, $req)
{
    switch ($req->method) {
        case 'GET':
            ...
            if (isset($req->query_params["activity"])) {
                return GetHouse($con, $req->query_params["activity"]);
            }

            return GetHouses($con);
...
function GetHouses($con, $options = null)
{

    // $condition="WHERE deleted IS NULL";

    $Activities = GetResource($con, "prayer_houses", "*");
...
function GetHouse($con, $slug)
{
    ...
    $Activities = GetResource($con, "prayer_houses", "*", $condition);
