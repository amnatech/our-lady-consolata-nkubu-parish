...
if ($dir != false) {
    ...
    // insert 
    $insert = "INSERT INTO activities (activity_id,title,subtitle,description,venue,duration,images_uri,slug,status) 
    VALUES('$activity_id','$title','$subtitle','$description','$venue','$duration','$dir','$slug','$status')";
    ...
    $res->message = "There Was An Error Uploading Activity Files";
}

$response=new stdClass();

$response->message="Create Activity";
...
function upload_gallery_images($files, $name)
{
    ...
    $oldmask = umask(0);
    mkdir('./uploads/activities/' . $folder_name . '/');
    umask($oldmask);

    $base_dir = "uploads/activities/" . $folder_name . '/';
    ...
    foreach ($files as $file) {
        ...
        if ($uploadOk == 0) {
            ...
            // print_r($file);
            if (move_uploaded_file($file['tmp_name'], 'uploads/activities/' . $folder_name . '/' . $image_name)) {
