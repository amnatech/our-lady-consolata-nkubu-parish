...
if ($dir != false) {
    ...
    // insert 
    $insert = "INSERT INTO gallery (album_id,title,album_date,description,created_by,images_uri,slug,visibility,status) 
    VALUES('$album_id','$title','$album_date','$description','$created_by','$dir','$slug','$visibility','$status')";
    ...
    $res->message = "There Was An Error Uploading Album Files";
}
...
function upload_gallery_images($files, $name)
{
    ...
    foreach ($files as $file) {
        ...
        if ($uploadOk == 0) {
            ...
            // print_r($file);
            if (move_uploaded_file($file['tmp_name'], 'uploads/gallery/' . $folder_name . '/' . $image_name)) {
